...
export function composeUserAgent(browserMode?: boolean) {
    ...
    // filter out app-specific tags that may be added earlier
    const filteredUserAgent = baseUserAgent.filter((ua) => !ua.startsWith('Mattermost') && !ua.startsWith('PlatrumChat'));

    if (browserMode) {
        return filteredUserAgent.join(' ');
    }

    return `${filteredUserAgent.join(' ')} PlatrumChat/${app.getVersion()}`;
}
...
export function resetScreensharePermissionsMacOS() {
    ...
    return exec('tccutil reset ScreenCapture Platrum.Desktop',
        {timeout: 1000});
}
