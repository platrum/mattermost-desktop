...
export class MainWindow extends EventEmitter {
    ...
    init = () => {
        ...
        if (process.platform === 'linux') {
            // In packaged Linux targets, app_icon.png is usually placed near the executable.
            // Fall back to the source-tree path for dev/test runs.
            const resourcesPath = typeof process.resourcesPath === 'string' ? process.resourcesPath : '';
            windowOptions.icon = resourcesPath ?
                path.join(resourcesPath, '..', 'app_icon.png') :
                path.join(path.resolve(app.getAppPath(), 'assets'), 'linux', 'app_icon.png');
        }
